...
def convert_to_euler(pose_mat):
    yaw = (
        180 / math.pi * math.atan2(pose_mat[1][0], pose_mat[0][0])
    )  # pitch for tracker  #Changed math.atan to math.atan2 for yaw, pitch, roll
    pitch = (
        180 / math.pi * math.atan2(pose_mat[2][0], pose_mat[0][0])
    )  # yaw for tracker    #Changed coordinates to only rely on x and z coordinates for yaw
